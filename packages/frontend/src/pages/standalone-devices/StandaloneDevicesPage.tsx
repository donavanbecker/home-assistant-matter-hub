...
import { fetchEntityList, HomeAssistantEntity } from "../../api/home-assistant-entities";
import {
...
function StandaloneDevicesPage() {
  ...
  return (
    <div>
      ...
      <DeviceCreationForm onCreated={device => setDevices([...devices, device])} />
    </div>
  );
}
...
function DeviceCreationForm({ onCreated, initialValues }: { onCreated: (device: any) => void, initialValues?: Partial<{ name: string, port: number, deviceType: string, entities: string[] }> }) {
  ...
  const [entities, setEntities] = useState<string[]>(initialValues?.entities ?? []);
  const [entityInput, setEntityInput] = useState("");
  const [entitySuggestions, setEntitySuggestions] = useState<HomeAssistantEntity[]>([]);
  ...
  const isValidEntityId = (id: string) => /^([a-zA-Z0-9_]+\.[a-zA-Z0-9_]+)$/.test(id);
  ...
  return (
    <form onSubmit={handleSubmit} style={{ marginTop: 16, marginBottom: 32, maxWidth: 500 }}>
      <div style={{ marginBottom: 8 }}>
        <label title="A friendly name for this device (e.g. 'Living Room Lamp')">
          Name:
          <input value={name} onChange={e => setName(e.target.value)} /* required removed for test JS validation */ style={{ marginLeft: 8, width: 220, borderColor: !name.trim() && error ? 'red' : undefined }} maxLength={64} />
        </label>
      </div>
      <div style={{ marginBottom: 8 }}>
        <label title="TCP port for this device (1024-65535)">
          Port:
          <input type="number" value={port} min={1024} max={65535} onChange={e => setPort(Number(e.target.value))} required style={{ marginLeft: 8, width: 120, borderColor: (!isValidPort(port) && error) ? 'red' : undefined }} />
        </label>
      </div>
      <div style={{ marginBottom: 8 }}>
        <label title="The type of Matter device to create (e.g. Light, Lock, Fan)">
          Device Type:
          <select value={deviceType} onChange={e => setDeviceType(e.target.value)} required style={{ marginLeft: 8, width: 260, borderColor: (!deviceType && error) ? 'red' : undefined }}>
        ...
        <label title="Add Home Assistant entity IDs (auto-complete, e.g. 'light.living_room')">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', marginLeft: 8 }}>
            <input
              ...
              style={{ width: 300, borderColor: (entities.length === 0 && error) ? 'red' : undefined }}
              onKeyDown={e => {
                if (e.key === 'Enter' && entityInput.trim()) {
                  e.preventDefault();
                  if (isValidEntityId(entityInput.trim()) && !entities.includes(entityInput.trim())) {
            ...
            <datalist id="entity-suggestions">
              {entitySuggestions.map((ent) => (
                <option key={ent.entity_id} value={ent.entity_id}>{ent.friendly_name || ent.entity_id}</option>
              ))}
            </datalist>
            <div style={{ fontSize: 12, color: '#888', marginTop: 2 }}>
              Example: <code>light.living_room</code>
              {entityLoading && <span style={{ marginLeft: 8 }}>Loading...</span>}
            </div>
            <div style={{ marginTop: 4 }}>
              {entities.map((id) => (
                <span key={id} style={{
                  display: 'inline-block',
                  background: invalidEntities.includes(id) ? '#ffdddd' : '#e0e0e0',
                  ...
                  marginBottom: 2,
                  fontSize: 13,
                  border: invalidEntities.includes(id) ? '1px solid red' : 'none',
                  cursor: 'pointer',
                }}
                  title={invalidEntities.includes(id) ? 'Invalid entity ID' : 'Click to remove'}
                  onClick={() => setEntities(entities.filter(eid => eid !== id))}
                >
                  {id} ×
                </span>
              ))}
      ...
      {error && <div style={{ color: "red", marginTop: 8 }} data-testid="device-form-error">{error}</div>}
